...
async def enhance_audio_prompt(user_text: str) -> str:
    ...
    try:
        ...
        log(f"[PromptEnhancer] Вызываем предобученную модель (Responses API)...")
        log(f"[PromptEnhancer] Model ID: {AUDIO_PROMPT_ENHANCER_ID}")
        log(f"[PromptEnhancer] Input text: {user_text}")
        
        # Вызываем предобученную модель в отдельном потоке
        def _call_openai():
            response = client.responses.create(
                prompt={
                    "id": AUDIO_PROMPT_ENHANCER_ID,
                    "version": AUDIO_PROMPT_ENHANCER_VERSION
                },
                input=[{
                    "role": "user",
                    "content": user_text
        ...
        if not enhanced_text:
            log(f"[PromptEnhancer] ⚠️ Не удалось извлечь текст из ответа, используем оригинал")
            return user_text

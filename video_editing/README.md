...
- `render/timeline/config/blocks.json` — конфигурация дополнительных блоков для интро/аутро и оверлеев (по умолчанию пуст, заполняется пользователем при необходимости).
...
По умолчанию без `--templates` рендерится только `mix_basic_circle`. Если нужны другие пресеты, перечисли их явно через `--templates overlay,circle,...`.
...
- **Кеширование оверлеев**: сгенерированные оверлеи сохраняются в Cloudflare R2 на 24 часа. При генерации нового видео кеш обнуляется автоматически, а повторные запуски используют уже готовые оверлеи. В Telegram-боте есть команда `/overlay`, чтобы принудительно пересоздать оверлей.
- **Ограничение скорости видео**: Shotstack API ограничивает максимальную скорость видео до 10x. Если требуется большее ускорение (например, при очень коротком фоновом видео), скорость автоматически ограничивается до 10x, а длительность клипа корректируется соответственно.

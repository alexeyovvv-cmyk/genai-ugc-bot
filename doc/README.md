...
#### **openai_enhancement_service.py** - Анализ эмоций
- Разбивка текста на сегменты по эмоциям через OpenAI fine-tuned модель
- Поддержка формата `[emotion] text` с множественными эмоциями
- Корректный парсинг эмоций на одной строке и с переносами
- Нормализация эмоций для MiniMax TTS API

#### **minimax_service.py** - TTS с эмоциями
- Синтез речи через MiniMax Speech-02 Turbo (fal.ai)
- Поддержка эмоциональной окраски голоса
- Генерация по сегментам с разными эмоциями
- Автоматическая склейка аудио сегментов
...
- 🎭 **Фикс парсинга эмоций** - корректная обработка множественных эмоций в одной строке
- ⚡ **Ограничение скорости видео** - автоматическое ограничение до 10x (лимит Shotstack API)
